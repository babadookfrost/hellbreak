...
const EVOLUTIONS = {
  ...
  void_shard: { affix: "splash", target: "void_shard_evo" }, // Assuming splash or similar for void_shard. Will trigger if they get any evolution item if we just match generic items, but the game checks for specific affixes.
};
function tryEvolveWeapon(Game, pickedItem) {
  ...
  if (evoRules) {
    ...
    if (
      pickedItem.pos.id === evoRules.affix ||
      pickedItem.neg.id === evoRules.affix
    ) {
      ...
      spawnBanner(Game, {
        ...
        subtitle: w.icon + " " + w.name.toUpperCase(),
        color: "#e8a317",
      });
...
function playSoundShoot(weaponId) {
  ...
  const sfxVol = typeof getSFXVolume === "function" ? getSFXVolume() : 0.5;
...
function maybeUnlockWeapon(Game) {
  ...
  const cfg = Game.levelCfg,
    idx = cfg.weaponWaves.indexOf(floorWaves);
  if (idx === -1) return;
  const wid = cfg.weaponOrder[idx];
  if (Game.player.weaponId === wid) return;
  Game.player.weaponId = wid;
  const w = WEAPONS[wid];
  Game.player.ammo = Math.max(1, w.ammoMax + Game.stats.ammoAdd);
  Game.player.reloading = 0;
  spawnBanner(Game, {
  ...
  screenFlash(Game, 0.6);
  burst(Game, Game.player.x, Game.player.y, 30, "#e8a317", 300);
}
...
function fireWeapon(Game, ax, ay) {
  ...
  p.cd = w.cd;
  if (GameRNG.random() >= Game.stats.ammoSave) p.ammo--;
  Game.shotSlowmo = 0.5;
  if (p.ammo === 0) p.reloading = w.reload * Game.stats.reloadMul;
  ...
  for (let i = 0; i < w.pellets; i++) {
    ...
    Game.bullets.push({
      ...
      isRailgun: w.id === "storm_staff_evo", // Keep railgun logic for lightning staff evo
      isInferno: p.weaponId === "inferno",
      isIce: p.weaponId === "winter_wand",
      isCursedEvo: p.weaponId === "cursed_blade_evo",
      isVoidEvo: p.weaponId === "void_shard_evo",
    });
  }
  muzzleFlash(Game, p.x, p.y, ang);
  screenShake(
    Game,
    w.splash ? 8 : w.id === "shotgun" || w.id === "sweeper" ? 5 : 2,
  );
  playSoundShoot(w.id);
}
...
function reloadWeapon(Game) {
  const p = Game.player,
    w = WEAPONS[p.weaponId];
  if (p.ammo < Math.max(1, w.ammoMax + Game.stats.ammoAdd) && p.reloading <= 0)
    p.reloading = w.reload * Game.stats.reloadMul;
}
