  ...
  proposedContracts.forEach((c) => {
    if (c.id === "boss_specific_weapon") {
      ...
      const randomWeaponId =
        weaponKeys[Math.floor(GameRNG.random() * weaponKeys.length)];
...
function renderContractsList() {
  ...
  let stats = JSON.parse(
    localStorage.getItem("myasorubka_contracts_stats") || "{}",
  );

  proposedContracts.forEach((c) => {
    const el = document.createElement("div");
    const isSelected = selectedContractIds.has(c.id);
    const reachedMax = selectedContractIds.size >= 2;
    ...
    const stateClass = isSelected
      ...
      : reachedMax
        ? "unavailable"
        : "available";
    el.className = "universal-card " + stateClass;
    ...
    el.innerHTML = `
      <div class="card-content">
        ...
        <div class="text-small" style="color:#7a7a7a;">Выполнено раз: ${count}</div>
      </div>
...
function startGameWithContracts() {
  activeContracts = proposedContracts
...
function checkContractsOnLootPicked() {
  if (contractState.currentFloor === 1) {
    // 1 = Floor 2
    activeContracts.forEach((c) => {
      if (c.id === "floor_2_no_loot" && c.status === "active") {
        failContract(c);
...
function drawContractsHUD(ctx, Game) {
  ...
  activeContracts.forEach((c) => {
    ...
    let progress = "";
    if (c.status === "active") {
      if (c.id === "kill_50_one_wave")
        progress = ` (${contractState.killsThisWave}/50)`;
      if (c.id === "no_damage_60s")
        progress = ` (${Math.floor(contractState.timeWithoutDamage)}s/60s)`;
    }
