...
function compact(arr, dk) {
  let w = 0;
  for (let i = 0; i < arr.length; i++) if (!arr[i][dk]) arr[w++] = arr[i];
  arr.length = w;
}
function compactByLife(arr) {
  let w = 0;
...
function forNearby(g, x, y, cb) {
  const cx = Math.floor(x / CELL),
    cy = Math.floor(y / CELL);
  for (let ox = -1; ox <= 1; ox++)
    for (let oy = -1; oy <= 1; oy++) {
      const b = g.get(cx + ox + "," + (cy + oy));
...
function generateMap() {
  ...
  for (let byi = 0; byi < by; byi++)
    for (let bxi = 0; bxi < bx; bxi++) {
      ...
      const y0 = byi * BLOCK + 2,
        y1 = Math.min(byi * BLOCK + BLOCK, MAP_H - 1);
      const x0 = bxi * BLOCK + 2,
        x1 = Math.min(bxi * BLOCK + BLOCK, MAP_W - 1);
      for (let y = y0; y < y1; y++)
        for (let x = x0; x < x1; x++) {
          let wall =
            pat < 0.4
  ...
  for (let y = cy - 3; y <= cy + 3; y++)
    for (let x = cx - 3; x <= cx + 3; x++)
      if (x > 0 && y > 0 && x < MAP_W - 1 && y < MAP_H - 1) grid[idx(x, y)] = 0;
  return {
    grid,
    idx,
    pxW: MAP_PX_W,
...
function isWallTile(map, tx, ty) {
  if (tx < 0 || ty < 0 || tx >= MAP_W || ty >= MAP_H) return true;
  return map.grid[map.idx(tx, ty)] === 1;
}
function isWallWorld(map, x, y) {
...
function moveWithCollision(map, e, dx, dy) {
  if (dx !== 0) {
    const nx = clamp(e.x + dx, TILE * 0.6, MAP_PX_W - TILE * 0.6);
    if (!circleBlocked(map, nx, e.y, e.r)) e.x = nx;
  }
  if (dy !== 0) {
    const ny = clamp(e.y + dy, TILE * 0.6, MAP_PX_H - TILE * 0.6);
    if (!circleBlocked(map, e.x, ny, e.r)) e.y = ny;
  }
}
function raycastClear(map, x0, y0, x1, y1) {
...
class Camera {
  ...
  follow(t, vw, vh, dt) {
    const tx = clamp(t.x - vw / 2, 0, Math.max(0, MAP_PX_W - vw));
    const ty = clamp(t.y - vh / 2, 0, Math.max(0, MAP_PX_H - vh));
...
function burst(Game, x, y, count, color, speed, gravity) {
  if (window.SettingsManager && SettingsManager.gfx === "low")
    count = Math.ceil(count / 2);
  for (let i = 0; i < count; i++) {
    const a = GameRNG.random() * Math.PI * 2;
    const sp = (0.3 + GameRNG.random() * 0.7) * speed;
    Game.particles.push({
      x,
      y,
      vx: Math.cos(a) * sp,
      vy: Math.sin(a) * sp,
      life: 0.5 + GameRNG.random() * 0.4,
      color,
      size: 2 + GameRNG.random() * 5,
      gravity: gravity || 0,
    });
  }
}
...
function screenFlash(Game, intensity) {
  Game.flash = Math.max(Game.flash || 0, intensity);
}
function screenShake(Game, amount) {
  Game.shake = Math.max(Game.shake || 0, amount);
}
function spawnBanner(Game, opts) {
  Game.banners.push({
    title: opts.title,
    subtitle: opts.subtitle,
    color: opts.color || "#e8a317",
    t: 0,
    duration: 2.0,
  });
}
function spawnFloatingText(Game, x, y, text, color) {
  Game.floatTexts.push({
    ...
    color,
    life: 1.0,
    vy: -60 + GameRNG.random() * -40,
    vx: (GameRNG.random() - 0.5) * 30,
    t: 0,
  });
}
function spawnImpact(Game, x, y, color, size) {
  Game.impacts.push({
    x,
    y,
    r: size || 20,
  ...
  burst(Game, x, y, 8, color || "#fff", 150, 200);
}
...
function startAmbientMusic() {
  ...
  const vol =
    window.SettingsManager && typeof SettingsManager.musicVol === "number"
      ? SettingsManager.musicVol
      : 0.5;
  ...
  ambientGain.gain.setValueAtTime(vol * 0.2, audioCtx.currentTime); // Base ambient volume
  ...
  ambientOsc.start();
  updateAmbientMusic(Game.floorIndex || 0);
}

function updateAmbientMusic(floorIndex) {
  if (!ambientOsc || !ambientFilter) return;
  // Floor-based ambient changes
  // Fridge: lower pitch, muffled
  // Core: louder, slightly higher pitch, more open filter
  const floorType = floorIndex % 4; // Assuming FLOORS.length = 4 based on context

  if (floorType === 1) {
    // Fridge
    ambientOsc.frequency.setTargetAtTime(45, audioCtx.currentTime, 1);
    ambientFilter.frequency.setTargetAtTime(300, audioCtx.currentTime, 1);
  } else if (floorType === 2) {
    // Core/Boiler
    ambientOsc.frequency.setTargetAtTime(65, audioCtx.currentTime, 1);
    ambientFilter.frequency.setTargetAtTime(800, audioCtx.currentTime, 1);
  } else {
    // Default/Heart
    ambientOsc.frequency.setTargetAtTime(55, audioCtx.currentTime, 1);
    ambientFilter.frequency.setTargetAtTime(500, audioCtx.currentTime, 1);
  }
}
...
function updateAmbientVolume() {
  ...
  const vol =
    window.SettingsManager && typeof SettingsManager.musicVol === "number"
      ? SettingsManager.musicVol
      : 0.5;
  ambientGain.gain.setTargetAtTime(vol * 0.2, audioCtx.currentTime, 0.1);
}

function getSFXVolume() {
  return window.SettingsManager && typeof SettingsManager.sfxVol === "number"
    ? SettingsManager.sfxVol
    : 0.5;
}
function createPlayer(spawn) {
  const opId = Game.isDaily ? "recruit" : metaState.lastOperator || "recruit";
  const op =
    typeof OPERATORS !== "undefined" && OPERATORS[opId]
      ? OPERATORS[opId]
      : {
          hpMul: 1,
          dmgMul: 1,
  ...
  return {
    ...
    ammo: WEAPONS[op.weapon || "pistol"]
      ? WEAPONS[op.weapon || "pistol"].ammoMax
      : 10,
    reloading: 0,
    cd: 0,
    dashCd: 0,
...
function applyLevelStart(Game, isContinuation = false) {
  ...
  if (typeof updateAmbientMusic === "function")
    updateAmbientMusic(Game.floorIndex);

  if (!isContinuation) {
    if (!Game.player.op || !Game.player.op.weapon) {
      Game.player.weaponId = cfg.weaponOrder[0];
    }
    const w = WEAPONS[Game.player.weaponId];
    Game.player.ammo = Math.max(1, w.ammoMax + Game.stats.ammoAdd);
    Game.player.reloading = 0;
  }

  if (typeof checkContractsOnFloorStart === "function")
    checkContractsOnFloorStart(Game.level);
  if (typeof queueRadioMessage === "function")
    queueRadioMessage(Game.level % 4);
...
function maybeUnlockDash(Game) {
  ...
  spawnBanner(Game, {
    title: "НОВЫЙ НАВЫК",
    subtitle: "РЫВОК: SHIFT / КНОПКА",
    color: "#0ea5c7",
  });
  screenFlash(Game, 0.5);
  burst(Game, Game.player.x, Game.player.y, 35, "#0ea5c7", 350);
}
...
function tryDash(Game, dx, dy) {
  ...
  p.dashDY = dy / mag;
  p.dashT = 0.16;
  p.dashCd = 2.6 * Game.stats.dashCdMul;
  p.invuln = 0.24;
  burst(Game, p.x, p.y, 16, "#0ea5c7", 250);
  screenShake(Game, 3);
...
function updateBullets(Game, sdt) {
  const bullets = Game.bullets,
    targets = Game.boss ? Game.enemies.concat([Game.boss]) : Game.enemies;
  const tgrid = buildGrid(targets);
  for (let i = 0; i < bullets.length; i++) {
    ...
    if (b.isVoidEvo && !b.vortexTriggered) {
      // Check if we hit an enemy to trigger vortex before death
      let hit = false;
      forNearby(tgrid, b.x, b.y, (e) => {
        if (!e.dead && Math.hypot(b.x - e.x, b.y - e.y) < b.r + e.r) hit = true;
      });
      if (hit || isWallWorld(Game.map, b.x, b.y) || b.dead) {
        b.vortexTriggered = true;
        b.vx = 0;
        b.vy = 0;
        b.vortexTime = 0.4;
        b.dead = false;
    ...
    if (b.vortexTriggered) {
      ...
      for (let j = 0; j < targets.length; j++) {
        ...
        if (dist < 150 && dist > 10) {
          const pullForce = 150 * sdt;
          e.x += ((b.x - e.x) / dist) * pullForce;
          e.y += ((b.y - e.y) / dist) * pullForce;
        }
      }
      // Visual feedback
      if (GameRNG.random() < 0.3) {
        Game.particles.push({
          x: b.x + (GameRNG.random() - 0.5) * 150,
          y: b.y + (GameRNG.random() - 0.5) * 150,
          vx: 0,
          vy: 0,
          life: 0.3,
          color: "#4b0082",
          ...
          targetX: b.x,
          targetY: b.y,
          isVortexParticle: true,
        });
      }

      if (b.vortexTime <= 0) {
        b.dead = true;
        b.vortexTriggered = false; // Allow standard death processing/splash to occur
      } else {
        continue; // Wait for explosion
      }
    }
    if (b.dead) {
      if (b.isInferno) {
...
const Game = {
  ...
  effViewW: window.innerWidth,
  effViewH: window.innerHeight,
  map: null,
  camera: new Camera(),
  player: null,
  enemies: [],
  ...
  recalcStats() {
    const s = {
      dmgMul: 1 + metaState.dmgLvl * 0.05,
      pierceAdd: 0,
      dashCdMul: 1,
      speedMul: 1,
      slowTime: 0,
  ...
  die() {
    ...
    if (this.isDaily) {
      ...
      let earnedShards =
        Math.floor(this.kills * 0.5) + this.wave * 2 + this.level * 20;

      let contractsBonusRatio = 0;
      if (typeof activeContracts !== "undefined" && activeContracts) {
        const completed = activeContracts.filter(
  ...
  start() {
    ...
    this.loot = [];
    this.inventory = [];
    if (metaState.startItem > 0)
      this.inventory.push(generateLootItem("common"));
    this.stats = this.recalcStats();
    this.map = generateMap();
    this.player = createPlayer(this.map.spawnWorld);
    this.stats = this.recalcStats();
    this.player.hp = this.player.maxHp;
    ...
    this.camera.x = clamp(
      ...
      0,
      Math.max(0, this.map.pxW - this.viewW),
    );
    this.camera.y = clamp(
      this.player.y - this.viewH / 2,
      0,
      Math.max(0, this.map.pxH - this.viewH),
    );
    Input.cmd = false;
  },
  update(dt) {
    if (this.zones && this.zones.length > 0) {
      ...
      const tgrid = buildGrid(
        this.boss ? this.enemies.concat([this.boss]) : this.enemies,
      );
    ...
    if (this.state === "menu") {
      if (Input.clickUpgrades) {
        ...
        if (document.getElementById("upgrades-menu").style.display !== "none") {
          ...
          document.getElementById("operator-select-menu").style.display =
            "flex";
          renderOperatorsUI();
        }
      }
    ...
    if (this.state === "paused") {
      if (Input.keys["Escape"]) {
        ...
        this.last = this.lastRender / 1000;
    ...
    // Also handle escape for pause during play
    if (this.state === "play" && Input.keys["Escape"]) {
      Input.keys["Escape"] = false;
      this.state = "paused";
    ...
    const p = this.player,
      { mvX, mvY } = getMoveVector();
    if (Input.wantDash) {
      const ddx = Input.dashDirX || mvX,
        ddy = Input.dashDirY || mvY;
      tryDash(this, ddx, ddy);
      Input.wantDash = false;
      Input.dashDirX = 0;
      Input.dashDirY = 0;
    }
    if (this.hitstop > 0) {
    ...
    if (p.reloading > 0) {
      p.reloading -= dt;
      if (p.reloading <= 0) p.ammo = WEAPONS[p.weaponId].ammoMax;
    }
    if (Input.wantReload) {
      reloadWeapon(this);
      Input.wantReload = false;
    }
    if (Input.wantInventory) {
      showInventoryMenu();
      Input.wantInventory = false;
    }
    const mw = {
      x: Input.mouse.x + this.camera.x,
      y: Input.mouse.y + this.camera.y,
    };
    if (Input.fireReq) {
      fireWeapon(this, mw.x, mw.y);
      if (!WEAPONS[p.weaponId].auto) Input.fireReq = false;
    }
    ...
    if (this.zones) {
      for (let i = this.zones.length - 1; i >= 0; i--) {
        ...
        if (z.color === "#ff4400" && GameRNG.random() < 0.2) {
          this.particles.push({
            x: z.x + (GameRNG.random() - 0.5) * z.r * 1.5,
            y: z.y + (GameRNG.random() - 0.5) * z.r * 1.5,
            vx: 0,
            vy: -20 - GameRNG.random() * 20,
            life: 0.4 + GameRNG.random() * 0.3,
            color: "#ff4400",
            size: 3 + GameRNG.random() * 4,
          });
        }

        if (z.lastTick >= 0.5) {
          z.lastTick = 0;
          const targets = this.boss
            ? this.enemies.concat([this.boss])
            : this.enemies;
          for (let j = 0; j < targets.length; j++) {
            const e = targets[j];
            if (e.dead) continue;
            if (Math.hypot(z.x - e.x, z.y - e.y) < z.r) {
              ...
              spawnFloatingText(
                ...
                e.x,
                e.y - 20,
                (Math.round(z.dmg * 10) / 10).toString(),
                z.color || "#ff4400",
              );
              if (e.hp <= 0) killEnemy(this, e);
            }
    ...
    maybeUnlockWeapon(this);
    maybeUnlockDash(this);
    if (
      !this.boss &&
      !this.portal &&
      this.wave % 5 === 0 &&
      ...
      spawnBoss(this);
      if (typeof checkContractsOnBossSpawn === "function")
        checkContractsOnBossSpawn();
    ...
    if (this.portal && (this.state === "play" || this.state === "portal")) {
      if (!this.portalNotified) {
        if (typeof checkContractsOnPortalSpawn === "function")
          checkContractsOnPortalSpawn();
        this.portalNotified = true;
      }
      this.portalTimer -= dt;
      this.portalTriggerCd = Math.max(0, (this.portalTriggerCd || 0) - dt);
    ...
    if (this.loot) {
      ...
      for (let i = 0; i < this.loot.length; i++) {
        let l = this.loot[i];
        if (
          Math.hypot(p.x - l.x, p.y - l.y) <
          (p.r + l.r + 15) * this.stats.pickupMul
        ) {
          if (l.type === "hp") {
            if (p.hp < p.maxHp) {
              p.hp += 10;
            ...
            if (this.inventory.length < maxSlots) {
              ...
              if (typeof checkContractsOnLootPicked === "function")
                checkContractsOnLootPicked();
              this.stats = this.recalcStats();
              spawnBanner(this, {
                title: l.item.rarity.toUpperCase(),
                subtitle: l.item.pos.name + " & " + l.item.neg.name,
                color: l.item.color,
              });
    ...
    compact(this.enemies, "dead");
    compact(this.bullets, "dead");
    compactByLife(this.particles);
  ...
  draw() {
    ...
    ctx.save();
    const sx = (GameRNG.random() - 0.5) * this.shake * 2,
      sy = (GameRNG.random() - 0.5) * this.shake * 2;
    ctx.translate(
      sx - Math.round(this.camera.x),
      sy - Math.round(this.camera.y),
    );
    ...
    if (this.flash > 0.01) {
      ...
      if (this.flash > 0.3) {
        ctx.fillStyle = "rgba(255,0,0," + this.flash * 0.08 + ")";
        ctx.fillRect(0, 0, this.viewW * 0.03, this.viewH);
        ctx.fillRect(this.viewW * 0.97, 0, this.viewW * 0.03, this.viewH);
      }
    }
    ...
    if (this.shouts && this.shouts.length > 0) {
      ...
      for (let i = 0; i < this.shouts.length; i++) {
        ...
        if (shout.t <= shout.duration) {
          ...
          else if (shout.duration - shout.t < 0.5)
            alpha = (shout.duration - shout.t) / 0.5;
  ...
  loop(t) {
    const targetDt =
      window.SettingsManager && window.SettingsManager.fps
        ? 1000 / window.SettingsManager.fps
        : 0;
    if (targetDt > 0 && t - (this.lastRender || 0) < targetDt) {
...
function drawWorldEntities() {
  ...
  for (let i = 0; i < Game.bullets.length; i++) {
    ...
    ctx.globalAlpha = 1;
    ctx.shadowBlur = 14;
    ctx.shadowColor = b.color || "#e8a317";
  ...
  for (let i = 0; i < elist.length; i++) {
    ...
    if (
      ...
      ctx.lineTo(
        e.x + Math.cos(ang) * dashDist,
        e.y + Math.sin(ang) * dashDist,
      );
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.lineCap = "butt";
    ...
    const eRot =
      e.vx !== undefined
        ...
        : e.dashTarget
          ? Math.atan2(e.dashTarget.y - e.y, e.dashTarget.x - e.x)
          : 0;
    ctx.save();
    ...
    if (e.flash > 0) {
      ...
      ag.addColorStop(0, bc + "44");
      ag.addColorStop(1, bc + "00");
      ctx.fillStyle = ag;
      ctx.beginPath();
      ctx.arc(
        0,
        0,
        e.isHeart
          ? e.r * (1.5 + Math.sin(Date.now() / 150) * 0.2) * 2
          : e.r * 2,
        0,
        Math.PI * 2,
      );
  ...
  if (Game.portal) {
    ...
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.ellipse(px, py, 40, 20 + Math.sin(pt) * 5, 0, 0, Math.PI * 2);
    ctx.stroke();
  ...
  pg.addColorStop(0, "rgba(232,163,23,0.18)");
  pg.addColorStop(1, "rgba(232,163,23,0)");
  ctx.fillStyle = pg;
  ctx.beginPath();
  ...
  ctx.fillStyle =
    p.dashT > 0 ? "#0ea5c7" : p.invuln > 0 ? "#0ea5c7" : baseColor;

  const mw = {
    x: Input.mouse.x + Game.camera.x,
    y: Input.mouse.y + Game.camera.y,
  };
  ...
  if (!isMobile) {
    ...
    ctx.strokeStyle = "rgba(232,163,23,0.3)";
    ctx.lineWidth = 1.5;
    ctx.setLineDash([5, 5]);
    ctx.beginPath();
    ctx.moveTo(p.x, p.y);
...
if (typeof module !== "undefined" && module.exports) {
  module.exports.compactByLife = compactByLife;
}
