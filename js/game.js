  ...
  for(let i=0;i<bullets.length;i++){
    ...
    if(b.friendly){
      forNearby(tgrid,b.x,b.y,e=>{
        if(e.dead||b.dead)return;const rr=b.r+e.r;const dx=b.x-e.x,dy=b.y-e.y;if(dx*dx+dy*dy<rr*rr){
      ...
      if(b.splash&&b.dead){
        for(let j=0;j<targets.length;j++){
          const e=targets[j];if(e.dead)continue;const dx=b.x-e.x,dy=b.y-e.y;if(dx*dx+dy*dy<b.splash*b.splash){
            if (e.isBoss && e.state === 'invulnerable') {
                spawnFloatingText(Game,e.x,e.y-30,'БЛОК','#0ea5c7');
                continue;
            }
